...
import ModalOff from "../components/modalOfertar.component";
...
function LightEcommerceB(props) {
  ...
  const [modalOff, setModalOff] = React.useState({
    show: false,
  });
  //Esta logeado
              ...
              <div className="flex">
                ...
                {stateLogin ?
                  btn ?
                    ...
                    <>
                      <button
                        className={`flex ml-auto text-white text-center items-center bg-${props.theme}-500 border-0 py-2 px-6 focus:outline-none hover:bg-${props.theme}-600 rounded mr-2`}
                        disabled={btn}
                        onClick={async () => {
                          comprar();
                        }}
                      ...
                      <button
                        className={`flex ml-auto text-white text-center items-center bg-${props.theme}-500 border-0 py-2 px-6 focus:outline-none hover:bg-${props.theme}-600 rounded`}
                        onClick={() => {
                          setModalOff({
                            ...modal,
                            show: true,
                            tokenId: state?.tokens.tokenID,
                            contract: state?.tokens.contract,
                            collection: state?.tokens.collection,
                            collectionID: state?.tokens.collectionID,
                            title: "Hacer una oferta",
                            currency: currencys[parseInt(localStorage.getItem("blockchain"))],
                            blockchain: parseInt(localStorage.getItem("blockchain")),
                            message:
                              "Ingresa el costo de la oferta que deseas realizar por este NFT.",
                            buttonName: "Cancelar",
                            change: setModalOff,
                          });
